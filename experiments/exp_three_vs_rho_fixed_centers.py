"""
Evaluates the geometric response of eps_c against kappa for the three-layer 
nested KS constraint under fixed monitoring centers.
...
def main() -> None:
    ...
    eps_list = []
    for rho in rho_list:
        print(f"\n=== Three-layer (fixed centers) | rho = {rho} ===")
    ...
    plt.title(f"Three-layer (fixed centers): epsilon_c vs rho (rmin={rmin})")
    plt.grid(True)
    save_figure("three_vs_rho_fixed_centers")
    plt.close()
